    ...
    async function invoke() {
      ...
      if (!result.success) {
       ...
       dispatch(
        ...
        userActions.setUserImg({imgUrl: result.data.imgUrl})
                ...
                {photo ? (
                  photo && (
                    <img
                      src={photo}
                      alt="Profile photo"
                      className="absolute inset-x-0 top-0 mx-auto cursor-pointer w-36 h-36 rounded-full border-4 border-white"
                    />
                  )
                ) : (
                  <svg

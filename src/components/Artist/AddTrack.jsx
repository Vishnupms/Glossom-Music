  ...
  return (
    ...
    <ArtistLayout>
      ...
      <form onSubmit={handleSubmit(upload)}>
        ...
        <div className="flex flex-col md:flex-row ml-5 justify-center gap-8">
          <div className="w-full md:w-1/2 max-w-md">
            ...
            <div className="mb-4">
  <label htmlFor="category" className="block text-gray-500 text-sm font-bold mb-2">
    Category
  </label>
  <select
    className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
...
<div className="mb-4">
  ...
  {errors?.language && <p className="text-red-600">{errors.language.message}</p>}
      ...
      <label htmlFor="dropzone-image" className="flex flex-col items-center justify-center w-full h-52 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-bray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600">
        {!imgProgress?  <div className="flex flex-col items-center justify-center pt-5 pb-6">
          
          <svg aria-hidden="true" className="w-10 h-10 mb-3 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"></path></svg>
          <p className="mb-2 text-sm text-gray-500 dark:text-gray-400"><span className="font-semibold">Click to upload</span> or drag and drop</p>
          <p className="text-xs text-gray-500 dark:text-gray-400">{imgName?imgName:"Upload the Image file" }</p>
        </div>:""}
      
        <input id="dropzone-image" type="file" className="hidden" name="imgFile"  accept="image/*"  onChange={handleImgChange}/>
      <div>{imgProgress?`${imgProgress}%`:""}</div>
      {imgProgress == 100?<div className="font-bold text-black"><h3 className="text-green-600">Image Uploaded Successfully</h3></div>:  <button
      ...
      <label htmlFor="dropzone-file" className="flex flex-col items-center justify-center w-full h-52 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-bray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600">
        {!audioProgress?  <div className="flex flex-col items-center justify-center pt-5 pb-6">
          
          <svg aria-hidden="true" className="w-10 h-10 mb-3 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"></path></svg>
          <p className="mb-2 text-sm text-gray-500 dark:text-gray-400"><span className="font-semibold">Click to upload</span> or drag and drop</p>
          <p className="text-xs text-gray-500 dark:text-gray-400">{audioName?audioName:"Upload the Audio file" }</p>
        </div>:""}
      
        <input id="dropzone-file" type="file" className="hidden" name="audioFile" accept="audio/*" onChange={handleAudioChange}/>
      <div>{audioProgress?`${audioProgress}%`:""}</div>
      {audioProgress == 100?<div className="font-bold text-black"><h3 className="text-green-600">Song Uploaded Successfully</h3></div>:  <button
          className="bg-blue-500 hover:bg-blue-700 text-white font-normal py-2 px-4 rounded "
